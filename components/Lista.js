...
import Servicio from './card/Servicio';
...
const Lista = (props) => {
    ...
    // Mutation para actualizar a los usuarios
    const [actualizarViaje] = useMutation(ACTUALIZAR_USUARIO);

    const { unidad } = props;
    const colorMant = unidad.servicio ? ' text-green-500 ' : ' text-red-500 '

    const notify = () => toast('placa actualizada.');
    ...
    return (
        <div className="flex ">
            <button onClick={(e) => Mantenimiento()}>
                <span className={"mx-4 material-symbols-outlined" + colorMant}>
                    handyman
                </span>
            </button>
            {
                        ...
                        <div className='w-full grid grid-cols-8 gap-2 text-center'>
                            <div className='flex my-auto py-1 '>
                                <button type='submit' onClick={notify}>
                                    <span className='mx-4 material-symbols-outlined'>
                                        edit
                                    </span>
                                    
                                </button>
                                <label className="mx-4 text-md font-semibold "> {unidad.unidad} </label>
                            </div>
